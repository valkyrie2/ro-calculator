...
- Added 34 entries from the 13-08 thROG history. thROG-confirmed data: the four Re-Dim Glacier Battle Pass armors — Antares (450589), Sirius (450590), Vega (450591), Regulus (450592) — each with per-2/per-3 refine scaling, refine 9/11/13 breakpoints, their two card combos and the full Grade D–A block; and Costume Ancient Giant Helm (401424), whose ATK/MATK +200 and +20% Boss damage are wrapped in `TIME[2026-11-11]` to match the in-game removal date
- Wired the enchant slots for all four Re-Dim Glacier armors: slot 2 = Star Cluster of Power/Stamina/Concentration/Creative/Spell/Wisdom Lv1–3, slot 3 = every skill-specific Glacier Flower Spell (179 options; the ATK/MATK/CRI/race/size/property orbs at 150–161 are excluded since slot 3 is the skill slot), slot 4 = Star of Master Archer/Mettle/Sharp/Speed/Spell/Vital/Spirit Lv1–5 plus the flat POW/STA/WIS/SPL/CON/CRT +3 orbs. All 238 aegis names were checked against item.json, and `re_dim_glacier.spec.ts` guards the range boundaries so an off-by-one can't silently point a slot at a non-existent enchant
- Added the Summer event enchants: Summer Heat Lv.1–3 (310844–310846, +5/10/15% melee and ranged physical damage) and Summer Breeze Lv.1–3 (310847–310849, +5/10/15% magic damage of all properties). Divine Pride lists no host item for them, so they are registered but not yet wired into any equipment's enchant slot — they need the event gear's aegis name to become selectable
- Added 2 Q-Pet eggs: Winter Maiden Egg (9038) and Elephant Egg (9064). Both grant item drop rate only, which the calculator does not model, so their scripts are empty
- Added 19 Battle Pass Season 3 / gacha items whose thROG entry does not exist on Divine Pride yet — values come from the default (iRO) dataset and should be re-checked when thROG data lands: S3 Serpent Knight Helm (401460), S3 Nyar's Familiar (420898) and -VG (420899, sets with all 52 Glacier/Dim Glacier weapons), S3 Issgard Boots/Shoes and their -VG versions (470452–470455), S3 Serpent Shadow Pendant/Earring (1270258–1270259), S3 Nyar Bleesing (Support) Lv2 (315390), Focused Cat Paw Physical/Magical Hat (401466–401467), Focused Kitten Shield (460177), Gangster Tactician Headset (420931), Rasgand's Soutane (450583), Wandering Thief Shoes (470457, Thief classes), Heretic Merchant Boots (470461, Merchant classes), Dark Scientist Robe (480899), Necklace of Mintaka (491087), Scythe of Ice Flame (520003) and Mugen Shuriken [3] (650066)
- The `[S3]` items are registered as `S3 ...` without the square brackets: `EQUIP[...]` parsing stops at the first `]`, so a bracketed prefix would silently kill every set bonus that references them
- Fixed Hit Physical Stone (Upper) (314662): its +6% combo required only Hit Physical Stone (Middle), but the thROG text requires Middle **and** Lower — it was granting the bonus on a half set
- Costume enchant stones needed no additions. The 13-08 page lists the _stone_ item IDs (1001487–1001489, 1002395–1002399, 1002642), but the calculator keys off the _enchant_ IDs, and all of those are already present: Hit Physical Stone Dual/Upper/Middle/Lower/Garment (314661–314665) plus the Abyss Chaser (312692), Troubadour & Trouvere (312693) and Hyper Novice (312694) garment stones, each on its existing subtype (71 Upper / 72 Middle / 73 Lower / 74 Garment / 76 Dual)
- Not modeled from the 13-08 batch: cosmetic-only costumes and footprint effects (314813, 315285, 1002642, 1002984, 480810 and the Gatchaman collaboration set), pet eggs' drop rate, "Premium buff" conditional blocks, monster-specific damage lines on the S3 Issgard boots/shoes, "enables use of skill X" grants, and the Mammonite / generic "physical damage" per-refine lines on Scythe of Ice Flame, whose source text is too ambiguous to encode. Ten set-partner cards referenced by the new gear (Geffen Gang/Thief/Thug, Fei Kanabian, Midnight Fay Kanavian, Jormungandr Sanctum Guardian / Cult Warlock / Cult Bishop, RSX-0806, EL-A17T) are not in the calculator, so those combo lines stay inert until the cards are added
